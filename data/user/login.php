...
$sql = "SELECT uid FROM ma_user WHERE uname='$uname' AND upwd='$upwd'";
...
if(!$result){       //SQL语句执行失败
  ...
  if(!$row){        //用户名或密码错误
    ...
    if($pageToJump==='cart.html' && @$_SESSION['toBuyLid']){
      //完成购物车添加
      $sql = "SELECT iid FROM ma_shoppingcart_item WHERE user_id=$_SESSION[loginUid] AND product_id=$_SESSION[toBuyLid]";
      $result = mysqli_query($conn, $sql);
      if( mysqli_fetch_row($result) ){
        $sql = "UPDATE ma_shoppingcart_item SET count=count+1 WHERE user_id=$_SESSION[loginUid] AND product_id=$_SESSION[toBuyLid]";
      }else {
        $sql = "INSERT INTO ma_shoppingcart_item VALUES(NULL, $_SESSION[loginUid], $_SESSION[toBuyLid], $_SESSION[toBuyCount],false)";
      }
      $result = mysqli_query($conn, $sql);
